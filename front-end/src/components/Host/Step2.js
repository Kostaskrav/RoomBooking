...
class Step2 extends React.Component {
    ...
    Handle_to_date_time=(item,index_num)=>{
        ...
        if(item.Date_from!==null){
           ...
           return(
            <TextField
                value={this.handle_null_date(item.Date_to)}
                // required 
                label="Εως"
                type="date"
                name="Date_to"
                ...
                InputProps={{inputProps: { min: item.Date_from.toISOString().slice(0,10)} }}
    ...
    Handle_from_date_time=(item,index_num)=>{
        if(item.Date_to!==null){
           ...
           return(
            <TextField
                // required
                value={this.handle_null_date(item.Date_from)}
                name="Date_from"
                label="Από"
                type="date"
                // defaultValue="2017-05-24T10:30"
                InputProps={{inputProps: { min: new Date().toISOString().slice(0,10),max: item.Date_to.toISOString().slice(0,10)} }}
                onChange={(e)=>this.props.handleChangeList(e,index_num)}
                InputLabelProps={{
                shrink: true,
            }}/>
